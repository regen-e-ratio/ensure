 ...
 * client), which mints a one-time grant to the caller's OWN verified address so they receive exactly
 * the email a contact would and can open the view-once link themselves — building trust before
 * arming. The CTA is DISABLED/guarded with an explanation when the caller has no verified contact and
...
export function TestReleaseCta({ hasVerifiedContact }: TestReleaseCtaProps) {
  const [phase, setPhase] = useState<Phase>({ kind: "idle" });

  async function handleClick() {
    if (!hasVerifiedContact) return; // guarded — never call the endpoint without a verified contact.
  ...
  return (
    <div className="test-release">
      <button
        ...
        disabled={!hasVerifiedContact || phase.kind === "working"}
      >
      ...
      {hasVerifiedContact ? (
        <p className="test-release__hint">
          We&apos;ll email your own verified address exactly what your contacts would receive. The
          link can be opened only once.
        </p>
      ) : (
        <p className="test-release__hint">
          Add and verify a contact first — then you can send yourself a preview to see exactly what
